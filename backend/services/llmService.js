...
3. SLIDE CATEGORY: Every slide MUST include a "slide_category" field. Choose one:
   ...
   - "informational": For text-heavy explanations and comparisons. FILL each text area completely:
     ...
     * "steps": each step's "text" field MUST be 10-15 words explaining what happened.
     * "items": each item's "item_text" or "desc" MUST be 15-25 words of real explanatory content.
    ...
    while (retryCount <= 2) {
        ...
        try {
            ...
            if (e.status === 400 || e.message.includes('Model unloaded') || e.message.includes('fetch failed')) {
                throw new Error("LLM Error: The AI model was unloaded from memory or disconnected. Please reload the model in LM Studio / A1111 and try again.");
            }
